...
usage() {
  cat <<'EOF'
Usage: scripts/install-my-slack-codex.sh

Builds my-slack and installs the binary and skill assets into CODEX_HOME.
...
SOURCE_EXAMPLE_PATH="${REPO_ROOT}/docs/${SKILL_NAME}/my-slack-example.yaml"
...
render_skill() {
  local tmp_path
  tmp_path="$(mktemp "${TMPDIR:-/tmp}/my-slack-skill.XXXXXX")"

  MY_SLACK_BIN_PATH="${INSTALL_BIN_PATH}" MY_SLACK_README_PATH="${SOURCE_README_PATH}" perl -0pe '
    s/If it is not on `PATH`, use the provided absolute binary path\./sprintf(q{If it is not on `PATH`, use `%s`.}, $ENV{MY_SLACK_BIN_PATH})/e;
    s{If you need build, test, or lint instructions, read \[README\.md\]\(\.\./\.\./src/cmd/my-slack/README\.md\)\.}{sprintf(q{If you need build, test, or lint instructions, read `%s`.}, $ENV{MY_SLACK_README_PATH})}e;
    s/If the binary is not on `PATH`, replace `my-slack` with the provided absolute path\./sprintf(q{If the binary is not on `PATH`, replace `my-slack` with `%s`.}, $ENV{MY_SLACK_BIN_PATH})/e;
...
printf 'Add %s to PATH if you want to run my-slack directly from the shell.\n' "${INSTALL_BIN_DIR}"
