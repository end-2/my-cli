...
usage() {
  cat <<'EOF'
Usage: scripts/install-my-discord-codex.sh

Builds my-discord and installs the binary and skill assets into CODEX_HOME.
...
SKILL_NAME="my-discord"
SOURCE_SKILL_PATH="${REPO_ROOT}/docs/${SKILL_NAME}/SKILL.md"
SOURCE_EXAMPLE_PATH="${REPO_ROOT}/docs/${SKILL_NAME}/my-discord-example.yaml"
...
INSTALL_CONFIG_PATH="${INSTALL_CONFIG_DIR}/my-discord.yaml"
...
render_skill() {
  local tmp_path
  tmp_path="$(mktemp "${TMPDIR:-/tmp}/my-discord-skill.XXXXXX")"

  MY_DISCORD_BIN_PATH="${INSTALL_BIN_PATH}" MY_DISCORD_README_PATH="${SOURCE_README_PATH}" perl -0pe '
    s/If it is not on `PATH`, use the provided absolute binary path\./sprintf(q{If it is not on `PATH`, use `%s`.}, $ENV{MY_DISCORD_BIN_PATH})/e;
    s{If you need build, test, or lint instructions, read \[README\.md\]\(\.\./\.\./src/cmd/my-discord/README\.md\)\.}{sprintf(q{If you need build, test, or lint instructions, read `%s`.}, $ENV{MY_DISCORD_README_PATH})}e;
    s/If the binary is not on `PATH`, replace `my-discord` with the provided absolute path\./sprintf(q{If the binary is not on `PATH`, replace `my-discord` with `%s`.}, $ENV{MY_DISCORD_BIN_PATH})/e;
...
printf 'Add %s to PATH if you want to run my-discord directly from the shell.\n' "${INSTALL_BIN_DIR}"
